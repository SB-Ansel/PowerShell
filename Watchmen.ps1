...
function IsItAlive(){
    ...
    if ($ProcessName -match "AnyDesk"){
        ...
        Write-Host "    AnyDesk already running. No further action required!"
        $host.ui.RawUI.ForegroundColor = $Color
    } elseif ($ProcessName -ne "AnyDesk") {
        Start-Process -FilePath 'FilePath' # Need manually enter file path to executable
        $host.ui.RawUI.ForegroundColor = "DarkRed"
        Write-Host '    AnyDesk not running. Automatic restart initiated!'
        $host.ui.RawUI.ForegroundColor = $Color
    }else {
        $host.ui.RawUI.ForegroundColor = "DarkRed"
...
while ($true) {
    ...
    Write-Host 'Watchmen :> Script last run at :'$Time
    $host.ui.RawUI.ForegroundColor = $Color
